...
def load_ovis_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    from datasets.ovis_api.ovos import OVOS

    timer = Timer()
    json_file = PathManager.get_local_path(json_file)
    with contextlib.redirect_stdout(io.StringIO()):
        ovis_api = OVOS(json_file)
    if timer.seconds() > 1:
    ...
    if dataset_name is not None:
        ...
        cat_ids = sorted(ovis_api.getCatIds())
        cats = ovis_api.loadCats(cat_ids)
...
"""
    ...
    vid_ids = sorted(ovis_api.vids.keys())
    ...
    #  'id': 2232}
    vids = ovis_api.loadVids(vid_ids)

    anns = [ovis_api.vidToAnns[vid_id] for vid_id in vid_ids]
    total_num_valid_anns = sum([len(x) for x in anns])
    total_num_anns = len(ovis_api.anns)
    ...
    logger.info("Loaded {} videos in OVIS format from {}".format(len(vids_anns), json_file))
...
def register_ovis_instances(name, metadata, json_file, image_root):
    ...
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="ovis", **metadata
    )
...
if __name__ == "__main__":
    ...
    json_file = "F://Downloads//OVIS//annotations_train.json"
    image_root = "F://Downloads//OVIS//Images//train"
    dicts = load_ovis_json(json_file, image_root, dataset_name="ovis_train")
